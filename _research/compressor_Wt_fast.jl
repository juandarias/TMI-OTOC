    ...
    for s ∈ step_start:step_end
        ...
        if loaded == false # Calculates W(t) from scratch
            ...
            if mod(s, save_each) == 0
                ...
                save_tensors("$(output_file)_step=$(s)", W_t, ϵ_c);
            end
        ...
        ### Calculate densities
        ρΛ = operator_density(W_t, normalized = true);
...
function main()
    ...
    ## Do compression
    comp_Wt, ϵ_c = compress_Wt(W_ti, U_dt, dt, Mi+1, Mf; svd_params = svd_params, var_params = var_params)
