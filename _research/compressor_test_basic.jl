...
const output_file = get_param!(args_dict, "output_file", "U_alpha=$(alpha)_t=$(tf)_dt=$(dt)_tol=$(tol_compr)_"*JOB_ID);
...
function compress_Ut(initial_mpo::MPO{T}, incr_mpo::MPO{T}, dt, step_start, step_end; var_params) where {T}
    U_mdt = MPO(copy(initial_mpo.Wi));
    for m ∈ step_start:step_end
        log_message("\n ##### Calculating U($(m*dt)) ##### \n"; color = :green)
        U_mdt = prod(U_mdt, incr_mpo; compress = true, var_params...)
        var_params[:seed] = U_mdt;
        log_message("\n Bond dimension of compressed U(t) : $(maximum(U_mdt.D)) \n"; color = :blue) 
...
comp_Ut = compress_Ut(U_initial, U_dt, dt, Mi+1, Mf; var_params = var_params)
