...
function rho_exact(U_t, U_dt, step_initial, step_final, tol_range, outfile; kwargs...)

    L = Int(log2(size(U_t, 1)));
    ...
    for (i, step) ∈ enumerate(step_initial:step_final)
        ...
        #= Instead of calculating the exponential at every step, recursively
        calculates U(t+dt) = U(t)*U(t), requiring only matrix multiplications =#

        U_t = U_t * U_dt;
        W_t_exact = U_t' * σʸᵢ(L, L) * U_t;
        #U_exact_mpo = operator_to_mpo(U_exact);
        #U_t = deepcopy(U_exact_mpo);
        #W_t = calc_Wt(U_exact_mpo, Y, L);
        W_t_mps = cast_mps(operator_to_mpo(W_t_exact));
...
function main()
    ...
    ## Initial unitary
    U_ti = exp(-im * (ti - dt) * Hermitian(Matrix(H_TMI)));
    U_dt = exp(-im * dt * Hermitian(Matrix(H_TMI)));

    ## Calculate operator density
    rho_exact(U_dt, U_ti, initial_step, final_step, tol_range, outfile; Dmax = Dmax);
