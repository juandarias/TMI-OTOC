...
mutable struct WII
    ...
    function WII(α::Float64, L::Int64, J::Float64, Bx::Float64, Bz::Float64, dt::Number, Ni::Int, Nj::Int; kac_norm::Bool = true) #! the actual calculation of WII can be splitted to an outer constructor
...
function update_Wt!(Wt::MPS, Udt::MPO)
    N = Wt.L;
    d = 4;
    local Ri
    for i ∈ 1:N
        ...
        # QR factorization
        if i != 1
            Atilde = Ri * reshape(UWU, (prod(size(UWU)[1:3]), :))
