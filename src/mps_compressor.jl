...
"""
function mps_compress_var(bigket::MPS, guessket::MPS; tol_compr::Float64 = 1e-6, Dmax::Int = 1000, rate=Float64 = 1.1, normalize::Bool = true, VERB::Int64 = 1)
    L = bigket.L;
    ϵ = 2;
    s = 0;
    phys_d = bigket.d;
    ...
    while abs(ϵ) > tol_compr
        ...
        for n in 1:L-1 # Left-right sweep
            VERB == 2 && log_message("Left sweep at site $(n), ", color = :blue);#
            ...
            ϵ = 1 - sum(tr(Mnew[:,d,:]'*Mnew[:,d,:]) for d ∈ 1:phys_d);
            VERB == 2 && log_message("with error $(round(abs(ϵ), sigdigits = 4))\n", color = :blue);#
        end

        for n in L:-1:1 # right-left sweep
            VERB == 2 && log_message("Right sweep at site $(n), ", color = :blue);#
