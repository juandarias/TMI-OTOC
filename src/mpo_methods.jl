...
function update_tensor!(mpo::MPO, tensor::Array{T, 4}, loc::Int) where {T}
    ...
    loc != 1 && (mpo.D[loc-1] = size(tensor)[1]);
    return nothing
end
...
function operator_to_mpo(operator; ϵmax::Float64=0.0, Dmax::Int=2^50) where {T<:Number}
    ...
    for k ∈ 1:L-1
        ...
        Uk, Sk, Vk = svd(Matrix(Ψk));
        Daux = Dcutoff(Sk, Dmax, ϵmax);
        Dright = min(length(Sk), Daux);
